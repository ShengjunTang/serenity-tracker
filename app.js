...
const zhCopy = {
  ...
  aaoi: {
    ...
    catalysts: ["市场回调时 Serenity 仍强调看公司 projection，而不是交易 Fed 概率", "她用 AAOI H1 2027 约 471M projection 作为例子", "Serenity 称 LITE、AAOI、SIVE 等 optical players 在 selloff 后回升合理", "Serenity 称自己从 28 美元开始持续加仓 AAOI", "Serenity 称市场应支持 AAOI 这类美国 AI 光学 domestic champion", "她将 AAOI 与 INTC、IQE、XFAB、MU、WOLF、SOI、SIVE 一起放进西方关键 AI 供应链框架", "她认为 AAOI reshoring 后若达到 471m/month projection，会是关键验证", "hyperscaler 升级到 1.6T 后，sovereign / Tier-2 AI DC 可能继续吸收 800G 产能", "AMD / NVDA 相关客户对话传闻，仍需核实", "如果 H1 明年光模块成为瓶颈，可能出现涨价和 margin expansion", "2027 年上半年产能 / 收入爬坡", "光学超级周期关注", "光学同业重估"],
    risks: ["加息概率上升和报告驱动的 CPO 时间线担忧仍可能压制高 beta 光学标的", "AMD / NVDA 对话仍是 analyst rumors，需要验证", "美国最大产能和垂直整合执行存在风险", "471m/month projection 的来源、单位和可实现性必须核实", "domestic champion 框架不能替代订单、良率、利润率和客户集中度验证", "如果规模化后 ASP 如市场预期下行，利润率上行逻辑会削弱", "市值目标较激进", "产能爬坡与客户时间需要验证", "光学标的整体波动很大"],
    nextChecks: ["核实 AAOI H1 2027 约 471M projection 的来源和口径", "核实 AAOI reshoring 状态、补贴、产能和 471m/month projection 原始来源", "观察 AI 半导体回调中光学标的是否保持相对强势", "跟踪 AAOI 订单、产能、客户和 800G / 1.6T ASP 是否继续验证 optical recovery", "核实 AAOI 产能目标和垂直整合表述", "检查最新客户集中度及 AMD / NVDA 传闻来源", "比较分析师对 800G / 1.6T ASP 的假设", "核实 H1 2027 收入预测", "检查产能扩张与客户承诺", "与 SIVE / FOCI 的风险收益对比"],
  },
  rpi: {
    theme: "AI 需求兑现 / 欧洲硬件",
  ...
  iqe: {
    ...
    catalysts: ["欧洲光子供应链重估", "VPEC epiwafer 涨价被 Serenity 解读为 IQE 和 Landmark 的正向 bottleneck read-through", "MTSI 投资 IQE 以锁定产能，被她作为产能重要性的证据", "此前成功 call 的动量", "可能与 XFAB / SOI 共振"],
    risks: ["更多是回顾性提及", "VPEC 涨价对 IQE 的传导需要确认产品重叠、订单和产能利用率", "Landmark 暴露需要单独验证", "可能已经大幅上涨", "需要重新确认当前基本面"],
    nextChecks: ["核实 VPEC 涨价通知和产品类别", "检查 IQE 是否在指引、订单或定价中体现 epiwafer pricing power", "核实 MTSI 投资 IQE 的条款和产能锁定细节", "检查大涨后的当前估值", "识别现有光子收入暴露", "与更新鲜的 XFAB thesis 对比"],
  },
  soi: {
    theme: "硅光衬底 / 欧洲政策受益",
  ...
  "power-800v-dc": {
    ...
    catalysts: ["NVDA 推动数据中心供电架构向 800V DC 迁移", "Morgan Stanley 称 NVDA 否认 800V DC 被推迟的报告", "Serenity 认为 SemiAnalysis 相关说法与 Computex 检查相反，CPO / 800V selloff 反应过度", "Serenity 把 800V DC 与 CPO 投资框架上升到支持 NVDA 和西方 AI 供应链安全", "她提到 FAU、InP、SiC / GaN、laser capacity 等上游 capacity 建设", "众包名单包括 IFNNY、ON、Lite-On、VICR、LFUS、VSH、ENPH、NVTS、POWI、AEHR、WOLF、XFAB、300376 等", "300376.SZ 被列入名单后上涨 20%，显示社媒关注会推动短线波动"],
    risks: ["Serenity 再次明确 300376 等标的是粉丝推荐，并非她自己的持仓或荐股", "Serenity 明确说这只是众包名单", "Morgan Stanley 与 SemiAnalysis 说法需要阅读原始报告后再判断", "national champion 框架会扩大主题边界，每个 800V DC / power name 仍需直接暴露验证", "800V DC 和 CPO 时间线可能因技术障碍、客户导入和报告分歧产生高波动", "名单里很多公司只是相邻暴露，800V DC 纯度不同", "小盘股可能先被注意力推动，而不是基本面兑现", "名单混合功率半导体、储能、燃料电池、设备和相邻标的，纯度较低"],
    nextChecks: ["阅读 Morgan Stanley 与 SemiAnalysis 对 NVDA 800V DC / CPO 时间线的原始表述", "逐一映射名单公司真实 800V DC 数据中心供电暴露", "确认哪些公司有 AI 数据中心客户，而不只是 EV / 工业电源暴露", "区分纯功率半导体受益者和相邻 / 无关标的", "区分真正服务 NVDA / AI 供应链安全的公司与仅有政策相邻叙事的公司", "在公司证据核实前，把 300376.SZ 的涨停视为波动提醒"],
  },
  "lpk-spacex": {
